...
import { Booking, PackageTier, PortfolioItem } from '@/lib/types';

interface PhotographerStudioViewProps {
  initialTab?: 'requests' | 'packages' | 'portfolio' | 'calendar' | 'deliveries' | 'profile';
...
export function PhotographerStudioView({
  ...
  const [activeTab, setActiveTab] = useState<'requests' | 'packages' | 'portfolio' | 'calendar' | 'deliveries' | 'profile'>(initialTab);

  // Photographer profile record
  const currentPhotographer = photographers.find((p) => p.id === currentUser.id) || photographers[0];
  ...
  const [newPkgDeliverables, setNewPkgDeliverables] = useState('300 Color-Graded High-Res Stills\nOnline Private Gallery Access\n1 Premium Hardcover Coffee Table Album\nDrone Aerial Cinematography');

  // New portfolio item modal form state
  const [showNewPortfolioModal, setShowNewPortfolioModal] = useState(false);
  const [newPortTitle, setNewPortTitle] = useState('');
  const [newPortCategory, setNewPortCategory] = useState('Wedding');
  const [newPortUrl, setNewPortUrl] = useState('https://images.unsplash.com/photo-1519741497674-611481863552?auto=format&fit=crop&w=1200&q=80');
  const [newPortCaption, setNewPortCaption] = useState('Golden hour sunset couple portraits');

  // Gallery Delivery Modal
  const [deliveryModalBooking, setDeliveryModalBooking] = useState<Booking | null>(null);
  ...
  const studioBookings = bookings.filter(
    (b) => b.photographer_id === currentPhotographer?.id || b.photographer_id === currentUser.id
  );
  ...
  const deliveredBookings = studioBookings.filter((b) => b.status === 'delivered');

  // My packages
  const myPackages = packages.filter(
    (p) => p.photographer_id === currentPhotographer?.id || p.photographer_id === currentUser.id
  );
  ...
  const handleSaveProfile = (e: React.FormEvent) => {
    ...
    updatePhotographerProfile(currentPhotographer.id, {
      ...
      gear: profileForm.gear.split(',').map((s) => s.trim()).filter(Boolean),
      honors: profileForm.honors.split('\n').map((s) => s.trim()).filter(Boolean),
      languages: profileForm.languages.split(',').map((s) => s.trim()).filter(Boolean),
      categories: profileForm.categories.split(',').map((s) => s.trim()).filter(Boolean),
      hero_images: profileForm.hero_images.split('\n').map((s) => s.trim()).filter(Boolean),
    });
  ...
  const studioMonthName = new Intl.DateTimeFormat('en-US', { month: 'long' }).format(new Date(studioCalYear, studioCalMonth, 1));
  const daysInStudioMonth = new Date(studioCalYear, studioCalMonth + 1, 0).getDate();
  const firstDayStudioWeekday = new Date(studioCalYear, studioCalMonth, 1).getDay();
  ...
  const studioDays = Array.from({ length: daysInStudioMonth }, (_, i) => {
    ...
    const hasBooking = studioBookings.some((b) => b.event_date === dateStr && (b.status === 'confirmed' || b.status === 'pending'));
        ...
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <h1 className="mt-2 font-serif text-2xl font-bold tracking-tight text-[#1A1A1A] sm:text-3xl">
                {currentPhotographer?.business_name || 'My Photography Studio'}
              </h1>
              <p className="mt-1 text-xs text-[#767471]">
                Manage booking requests, update portfolio works, configure collections, and maintain profile details.
              </p>
            </div>

            <div className="flex items-center gap-2">
              <span className="rounded-xl border border-[#D9D2C2] bg-[#FBF9F5] px-3.5 py-2 text-xs font-semibold text-[#1A1A1A]">
                Status: <strong className="text-[#2D6A4F]">Accepting Reservations</strong>
              </span>
            </div>
          </div>
          ...
          <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4 pt-4 border-t border-[#F0ECE1]">
            <div className="rounded-2xl border border-[#E8E2D2] bg-[#FBF9F5] p-4">
              <div className="text-[10px] uppercase font-bold text-[#767471]">Pipeline Value</div>
              <div className="mt-1 font-serif text-xl font-bold text-[#1A1A1A]">
                ₹{(studioBookings.reduce((acc, b) => acc + b.total_price, 0)).toLocaleString('en-IN')}
              </div>
              <div className="text-[10px] text-[#767471] mt-0.5">{studioBookings.length} total client inquiries</div>
            </div>

            <div className="rounded-2xl border border-[#E8E2D2] bg-[#FBF9F5] p-4">
              <div className="text-[10px] uppercase font-bold text-[#767471]">Pending Review</div>
              <div className="mt-1 font-serif text-xl font-bold text-[#997316]">
                {pendingRequests.length} Requests
              </div>
              <div className="text-[10px] text-[#767471] mt-0.5">Awaiting your response</div>
            </div>

            <div className="rounded-2xl border border-[#E8E2D2] bg-[#FBF9F5] p-4">
              <div className="text-[10px] uppercase font-bold text-[#767471]">Confirmed Shoots</div>
              <div className="mt-1 font-serif text-xl font-bold text-[#2D6A4F]">
                {confirmedBookings.length} Scheduled
              </div>
              <div className="text-[10px] text-[#767471] mt-0.5">Session passes issued</div>
            </div>

            <div className="rounded-2xl border border-[#E8E2D2] bg-[#FBF9F5] p-4">
              <div className="text-[10px] uppercase font-bold text-[#767471]">Artist Rating</div>
              <div className="mt-1 font-serif text-xl font-bold text-[#1A1A1A] flex items-center gap-1">
                <Star className="h-4 w-4 fill-[#C59B27] text-[#C59B27]" />
                {currentPhotographer?.rating.toFixed(1) || '4.9'} / 5.0
              </div>
              <div className="text-[10px] text-[#767471] mt-0.5">{currentPhotographer?.review_count || 120} Client reviews</div>
            </div>
          </div>
          ...
          <div className="mt-6 flex gap-2 overflow-x-auto border-t border-[#F0ECE1] pt-4 scrollbar-none">
            {[
              { id: 'requests', label: `Inquiries & Requests (${pendingRequests.length})` },
              { id: 'portfolio', label: `Manage Portfolio (${myPortfolio.length})` },
              { id: 'profile', label: 'Studio Profile & Bio' },
              { id: 'packages', label: `Pricing & Packages (${myPackages.length})` },
              { id: 'calendar', label: 'Availability Calendar' },
              { id: 'deliveries', label: `Delivered Galleries (${deliveredBookings.length})` },
            ].map((tab) => (
              <button
                key={tab.id}
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        {activeTab === 'requests' && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="font-serif text-xl font-bold text-[#1A1A1A]">Manage Booking Requests</h2>
                <p className="text-xs text-[#767471]">
                  Accept or decline incoming client reservations. Confirming locks the dates and creates a verified session pass.
                </p>
              </div>
            </div>
                ...
                {studioBookings.map((req) => (
                  ...
                  >
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 pb-4 border-b border-[#F0ECE1]">
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="font-mono text-xs font-bold text-[#C59B27]">{req.booking_code}</span>
                          <span
                        ...
                        <p className="text-xs text-[#767471]">{req.package_name}</p>
                      </div>
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-[10px] text-[#767471]">{req.duration_hours}h duration</div>
                      </div>
                    </div>
                    ...
                    <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs text-[#52504E]">
                      <div className="flex items-center gap-2">
                        ...
                        <span>{req.event_date} ({req.event_time_start} - {req.event_time_end})</span>
                      </div>
                      <div className="flex items-center gap-2 col-span-2">
                        <MapPin className="h-4 w-4 text-[#C59B27]" />
                        <span className="truncate">{req.venue_name}, {req.venue_address}</span>
                      </div>
                    </div>

                    {req.creative_notes && (
                      <div className="mt-3 rounded-xl bg-[#FBF9F5] p-3 text-xs text-[#52504E] border border-[#E8E2D2]">
                        <strong className="text-[#1A1A1A] block mb-0.5">Client Creative Vision:</strong>
                    ...
                    <div className="mt-4 pt-4 border-t border-[#F0ECE1] flex flex-wrap items-center justify-between gap-3">
                      <div className="text-xs text-[#767471]">
                        Client Contact: <strong>{req.customer_email || 'client@photobook.in'}</strong>
                      </div>
                      ...
                      {req.status === 'delivered' && (
                        <span className="text-xs text-[#2D6A4F] font-semibold flex items-center gap-1">
                          <CheckCircle2 className="h-4 w-4" /> Master Gallery Delivered (PIN: {req.delivered_gallery_pin || 'PB-7892'})
                        </span>
                      )}
                    </div>
                  </div>
        ...
        {activeTab === 'portfolio' && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="font-serif text-xl font-bold text-[#1A1A1A]">Manage Portfolio & Selected Works</h2>
                <p className="text-xs text-[#767471]">
                  Add your best photos, showcase specific genres, and curate your client catalog.
                </p>
              </div>
              <button
              ...
              {myPortfolio.map((item) => (
                ...
                >
                  ...
                  <div className="p-3">
                    ...
                    <div className="font-serif text-xs font-bold text-[#1A1A1A] truncate">{item.title}</div>
                    {item.caption && (
                      <p className="text-[10px] text-[#767471] truncate">{item.caption}</p>
                    )}
                  </div>
                </div>
              ))}
        ...
        {activeTab === 'profile' && (
          <div className="rounded-3xl border border-[#D9D2C2] bg-white p-6 sm:p-8 shadow-xs">
            <div className="border-b border-[#F0ECE1] pb-4 mb-6">
              <h2 className="font-serif text-xl font-bold text-[#1A1A1A]">Edit Studio Profile & Details</h2>
              <p className="text-xs text-[#767471]">
                Update your public profile, business branding, experience, gear list, and awards.
              </p>
            </div>
            ...
            <form onSubmit={handleSaveProfile} className="space-y-6">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  <label className="block text-xs font-bold text-[#1A1A1A] mb-1">Business / Atelier Name</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-bold text-[#1A1A1A] mb-1">Base City</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-bold text-[#1A1A1A] mb-1">State</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-bold text-[#1A1A1A] mb-1">Starting Session Price (₹)</label>
                  <input
                    type="number"
                    value={profileForm.starting_price}
                    onChange={(e) => setProfileForm({ ...profileForm, starting_price: Number(e.target.value) })}
                    required
                    className="w-full rounded-xl border border-[#D9D2C2] bg-[#FBF9F5] px-3.5 py-2.5 text-xs text-[#1A1A1A] focus:border-[#C59B27] focus:outline-none"
                  />
                </div>
                ...
                <div>
                  <label className="block text-xs font-bold text-[#1A1A1A] mb-1">Experience (Years)</label>
                  <input
                    type="number"
              ...
              <div>
                <label className="block text-xs font-bold text-[#1A1A1A] mb-1">Artist Biography / Studio Philosophy</label>
                <textarea
                  rows={4}
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) => setProfileForm({ ...profileForm, hero_images: e.target.value })}
                    className="w-full rounded-xl border border-[#D9D2C2] bg-[#FBF9F5] px-3.5 py-2.5 text-xs text-[#1A1A1A] focus:border-[#C59B27] focus:outline-none"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => setProfileForm({ ...profileForm, honors: e.target.value })}
        ...
        {activeTab === 'packages' && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="font-serif text-xl font-bold text-[#1A1A1A]">Pricing & Deliverable Tiers</h2>
                <p className="text-xs text-[#767471]">
                  Configure your transparent fixed price packages with clear turnaround times and deliverables.
                </p>
              </div>
              <button
                ...
                >
                  <div>
                    <div className="flex items-start justify-between">
                      <h3 className="font-serif text-lg font-bold text-[#1A1A1A]">{pkg.name}</h3>
                      {pkg.is_popular && (
                    ...
                    <p className="text-xs text-[#767471] mt-0.5">{pkg.tagline}</p>

                    <div className="mt-4 font-serif text-2xl font-bold text-[#1A1A1A]">
                      ₹{pkg.price.toLocaleString('en-IN')}
                    </div>
                    <div className="text-xs text-[#52504E] mt-0.5">{pkg.duration_hours} Hours Coverage • {pkg.turnaround_days}-day delivery</div>

                    <div className="mt-4 space-y-1.5 border-t border-[#F0ECE1] pt-4">
                      {pkg.deliverables.map((d, i) => (
                        <div key={i} className="text-xs text-[#52504E] flex items-center gap-1.5">
                  ...
                  <div className="mt-6 pt-4 border-t border-[#F0ECE1] flex justify-end">
                    <button
                      onClick={() => {
                        if (confirm('Delete this package tier?')) deletePackage(pkg.id);
                      }}
          ...
          <div className="rounded-3xl border border-[#D9D2C2] bg-white p-6 sm:p-8 shadow-sm">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
              <div>
                ...
                <p className="text-xs text-[#767471]">
                  Click on any open date to toggle between Available for Bookings and Blocked / Personal Shooting.
                </p>
              </div>
            ...
            <div className="grid grid-cols-7 gap-2 text-center">
              {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((d) => (
                <div key={d} className="py-1 text-[11px] font-bold text-[#767471] uppercase">
                  {d}
                </div>
              ))}
              ...
              {studioDays.map((cal) => (
                <button
                  ...
                  title={cal.isPast ? 'Past date' : cal.hasBooking ? 'Confirmed client shoot' : cal.isBlocked ? 'Blocked off' : 'Open for bookings'}
                >
                  <span className="text-sm font-bold">{cal.day}</span>
                  <span className="text-[9px] mt-0.5">
                    {cal.isPast ? 'Past' : cal.hasBooking ? 'Booked' : cal.isBlocked ? 'Blocked' : 'Open'}
                  </span>
                </button>
              ))}
            </div>
        ...
        {activeTab === 'deliveries' && (
          <div className="space-y-6">
            <div>
              <h2 className="font-serif text-xl font-bold text-[#1A1A1A]">Delivered Client Gallery Vaults</h2>
              <p className="text-xs text-[#767471]">
                View delivered client archives, generated security PINs, and photo download logs.
              </p>
            </div>

            {deliveredBookings.length === 0 ? (
              <div className="rounded-3xl border border-dashed border-[#D9D2C2] bg-white p-12 text-center text-xs text-[#767471]">
                No galleries delivered yet. Complete and deliver galleries from your Inquiries tab.
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {deliveredBookings.map((d) => (
                  <div key={d.id} className="rounded-2xl border border-[#E8E2D2] bg-white p-5 shadow-xs">
                    <div className="flex justify-between items-start">
                      <div>
                        <h4 className="font-serif text-base font-bold text-[#1A1A1A]">{d.customer_name}</h4>
                        <p className="text-xs text-[#767471]">{d.package_name}</p>
                      </div>
                    ...
                    <div className="mt-3 text-xs text-[#52504E]">
                      Event Date: {d.event_date} • {d.delivered_photos_count || 320} Master Photos Available
                    </div>
                  </div>
                ))}
              </div>
      ...
      {showNewPackageModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-xs">
          <div className="w-full max-w-lg rounded-3xl border border-[#D9D2C2] bg-white p-6 shadow-2xl">
            <h3 className="font-serif text-lg font-bold text-[#1A1A1A] pb-3 border-b border-[#F0ECE1]">
              Create Pricing Collection Tier
            </h3>

            <form onSubmit={handleCreatePackage} className="mt-4 space-y-4 text-xs">
              <div>
                <label className="block font-bold text-[#1A1A1A] mb-1">Tier Name</label>
                <input
                  type="text"
                  value={newPkgName}
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block font-bold text-[#1A1A1A] mb-1">Price (₹)</label>
                  <input
                    type="number"
                    value={newPkgPrice}
                ...
                <div>
                  <label className="block font-bold text-[#1A1A1A] mb-1">Coverage (Hours)</label>
                  <input
                    type="number"
                    value={newPkgHours}
                ...
                <div>
                  <label className="block font-bold text-[#1A1A1A] mb-1">Turnaround (Days)</label>
                  <input
                    type="number"
                    value={newPkgTurnaround}
                    onChange={(e) => setNewPkgTurnaround(Number(e.target.value))}
                    required
                    className="w-full rounded-xl border border-[#D9D2C2] bg-[#FBF9F5] p-2.5 text-xs text-[#1A1A1A] focus:border-[#C59B27] focus:outline-none"
                  />
                </div>
              ...
              <div>
                <label className="block font-bold text-[#1A1A1A] mb-1">Deliverables (one per line)</label>
                <textarea
                  rows={3}
      ...
      {showNewPortfolioModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-xs">
          <div className="w-full max-w-md rounded-3xl border border-[#D9D2C2] bg-white p-6 shadow-2xl">
            ...
            <form onSubmit={handleAddPortfolio} className="mt-4 space-y-4 text-xs">
              <div>
                <label className="block font-bold text-[#1A1A1A] mb-1">Frame Title</label>
                <input
                  type="text"
                  value={newPortTitle}
              ...
              <div>
                <label className="block font-bold text-[#1A1A1A] mb-1">Category</label>
                <select
              ...
              <div>
                <label className="block font-bold text-[#1A1A1A] mb-1">Image URL</label>
                <input
                  type="url"
                  value={newPortUrl}
              ...
              <div>
                <label className="block font-bold text-[#1A1A1A] mb-1">Caption / Story Note</label>
                <input
                  type="text"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-xs">
          <div className="w-full max-w-md rounded-3xl border border-[#D9D2C2] bg-white p-6 shadow-2xl">
            ...
            <p className="mt-2 text-xs text-[#767471]">
              Issue the encrypted private gallery link and access PIN for <strong>{deliveryModalBooking.customer_name}</strong>.
            </p>

            <div className="mt-4 space-y-4 text-xs">
              <div>
                <label className="block font-bold text-[#1A1A1A] mb-1">Delivered Master Photos Count</label>
                <input
                  type="number"
                  value={deliveryPhotoCount}
                  onChange={(e) => setDeliveryPhotoCount(Number(e.target.value))}
                  className="w-full rounded-xl border border-[#D9D2C2] bg-[#FBF9F5] p-2.5 text-xs text-[#1A1A1A] focus:border-[#C59B27] focus:outline-none"
                />
              </div>

              <div>
                <label className="block font-bold text-[#1A1A1A] mb-1">Client Security PIN</label>
                <input
                  type="text"
                  value={deliveryPin}
